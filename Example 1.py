...
# Define node coordinates for the frame
...
ops.node(5, 2 * girL, 0)    # Node 5 at (2 * girL, 0)
ops.node(6, 2 * girL, colL) # Node 6 at (2 * girL, colL)

# Define boundary conditions (supports)
ops.fix(1, 1, 1, 1)  # Fix all 3 DOFs at node 1
ops.fix(3, 1, 1, 0)  # Fix node 3 in x and y, allow rotation
ops.fix(5, 1, 1, 0)  # Fix node 5 in x and y, allow rotation
...
ops.element('elasticBeamColumn', 3, 5, 6, Acol, E, IzCol, 1)  # Column element 3 between nodes 5 and 6
ops.element('elasticBeamColumn', 4, 2, 4, Agir, E, IzGir, 1)  # Girder element 4 between nodes 2 and 4
ops.element('elasticBeamColumn', 5, 4, 6, Agir, E, IzGir, 1)  # Girder element 5 between nodes 4 and 6
...
Ew = {
    4: ['-beamUniform', Wy, Wx],  # Uniform distributed load on girder element 4
    5: ['-beamUniform', Wy, Wx]   # Uniform distributed load on girder element 5
}

# Apply point load at nodes 2 and 4 in the x-direction
ops.load(2, Px, 0.0, 0.0)  # Px applied at node 2
ops.load(4, Px, 0.0, 0.0)  # Px applied at node 4

# Apply distributed loads on the elements
for etag in Ew:
